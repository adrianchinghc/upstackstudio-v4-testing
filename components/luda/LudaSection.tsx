      ...
      <div className={cn('rounded-2xl bg-surface border border-default p-6 md:p-8', className)}>
        <div className="space-y-4">
          <div className="flex items-center gap-3">
            <div className="flex h-10 w-10 items-center justify-center rounded-lg bg-[var(--color-brand-navy)]/10 dark:bg-[var(--color-brand-sky)]/10">
              <Shield className="h-5 w-5 text-[var(--color-brand-navy)] dark:text-[var(--color-brand-sky)]" />
            </div>
            <div>
          ...
          <ul className="grid gap-2 text-sm text-secondary">
            {LUDA_PILLARS.map((pillar) => (
              <li key={pillar.title} className="flex items-start gap-2">
                <pillar.icon className="h-4 w-4 text-[var(--color-brand-sky)] shrink-0 mt-0.5" />
        ...
        <div className="grid md:grid-cols-2 gap-6 md:gap-8 mb-12 md:mb-16">
          {LUDA_PILLARS.map((pillar, index) => (
            <AnimatedSection
              key={pillar.title}
              delay={index * 0.1}
              className="group relative rounded-2xl border border-default bg-surface-2 p-8 md:p-10 transition-all hover:border-[var(--color-brand-sky)]/50"
            >
              <div className="flex items-start gap-4">
                <div className="flex h-14 w-14 items-center justify-center rounded-xl bg-[var(--color-brand-navy)]/10 dark:bg-[var(--color-brand-sky)]/10 shrink-0">
                  <pillar.icon className="h-7 w-7 text-[var(--color-brand-navy)] dark:text-[var(--color-brand-sky)]" />
                </div>
                <div>
